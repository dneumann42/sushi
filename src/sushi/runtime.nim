...
proc bindNativeCommands(env: Env) =
  for (name, implementation, signature, docString) in [
    ("if", ifCommand, "if condition do ... end [do ... end]", "Evaluates a condition and runs the then block, or an optional else block."),
    ("cond", condCommand, "cond do condition: value ... end", "Evaluates branch pairs until one condition is truthy, then returns its value."),
    ("while", whileCommand, "while condition do ... end", "Repeats a body block while the condition remains truthy."),
    ("let", letCommand, "let name value do ... end", "Binds a local name for the duration of a block."),
    ("var", varCommand, "var name value", "Defines a variable in the current scope."),
    ("set", setCommand, "set target value", "Updates an existing variable, instance field, list item, table entry, or array item."),
    ("eval", evalCommand, "eval value", "Evaluates one argument in the current scope."),
    ("eval-here", evalHereCommand, "eval-here value", "Evaluates captured syntax with the current scope layered over its captured scope."),
    ("eval-value", evalValueCommand, "eval-value value", "Resolves a raw value without invoking it as syntax."),
    ("raw", rawCommand, "raw value", "Returns the raw resolved value of one argument."),
    ("capture", captureCommand, "capture value", "Captures an argument together with its current environment."),
    ("capture-source", captureSourceCommand, "capture-source source", "Parses source text and captures the resulting top-level value."),
    ("replay", replayCommand, "replay block", "Replays a captured block in an appropriate replay scope."),
    ("eq", eqCommand, "eq left right", "Returns true when two evaluated values are equal."),
    ("not", notCommand, "not value", "Returns the boolean negation of a value's truthiness."),
    ("and", andCommand, "and left right", "Returns true when both arguments are truthy, short-circuiting the right side."),
    ("or", orCommand, "or left right", "Returns true when either argument is truthy, short-circuiting the right side."),
    ("read-line", readLineCommand, "read-line", "Reads one line from standard input, or false at end of input."),
    ("error", errorCommand, "error message", "Raises a Sushi error with the provided message."),
    ("error-at", errorAtCommand, "error-at target message", "Raises a Sushi error at the source span of a target value."),
    ("catch", catchCommand, "catch value [do ... end]", "Catches errors from an expression and optionally handles them in a block."),
    ("??", coalesceCommand, "?? value fallback", "Evaluates the left expression, or the right fallback when it raises."),
    ("!!", orElseQuitCommand, "!! value do ... end", "Evaluates the left expression, or runs an error block and exits when it raises."),
    ("++", concatCommand, "++ left right", "Concatenates two text values."),
    ("run", runCommand, "run source", "Evaluates Sushi source text in the current scope."),
    ("run-file", runFileCommand, "run-file path", "Loads and evaluates a Sushi file as an entry module."),
    ("fun", funCommand, "fun name [params] do ... end", "Defines a named function."),
    ("fn", fnCommand, "fn [params] body", "Creates an anonymous function."),
    ("class", classCommand, "class Name [Base] do ... end", "Defines a class with optional inheritance."),
    ("new", newCommandProc, "new Class [args...]", "Creates an instance of a class and runs its init method when present."),
    ("ctorCalled", ctorCalledCommand, "ctorCalled", "Returns the number of constructor calls made by the runtime."),
    ("use", useCommand, "use module [global]", "Loads a module and optionally imports its exports into the current module scope."),
    ("iterate", iterateCommand, "iterate item iterator do ... end", "Runs a block for each value produced by an iterator callable."),
    ("command-count", commandCountCommand, "command-count block", "Returns the number of commands in a block."),
    ("command-at", commandAtCommand, "command-at block index", "Returns a command from a block by index, preserving capture when needed."),
    ("object-count", objectCountCommand, "object-count command", "Returns the number of objects in a command."),
    ("object-at", objectAtCommand, "object-at command index", "Returns an object from a command by index, preserving capture when needed."),
    ("count", countCommand, "count list", "Returns the number of items in a list."),
    ("length", lengthCommand, "length value", "Returns the length of text, list, or array values."),
    ...
    (".", dotCommand, ". receiver member", "Performs member access or indexed access."),
    ("+", addCommand, "+ value [values...]", "Adds one or more numeric values."),
    ("-", subtractCommand, "- value [values...]", "Negates one numeric value or subtracts subsequent values from the first."),
    ("<", lessThanCommand, "< left right", "Returns true when the left number is less than the right number."),
    (">", greaterThanCommand, "> left right", "Returns true when the left number is greater than the right number."),
    ("array", arrayCommand, "array dimension [dimensions...]", "Creates a multidimensional array of integer zeroes."),
    ("assert", assertCommand, "assert condition", "Raises an error unless its argument evaluates to true.")
  ]:
    env.define(newSymbol(name), newNativeCommandValue(nativeCommandKind(
      name: name,
